...
* **Fixed: Disconnect did not disconnect.** Pressing it cleared the key and then WordPress's own settings-saving rules put it straight back, because the rule that keeps a password field you left alone cannot tell itself apart from a button whose whole job is clearing one. The site stayed connected however many times you pressed it.
* **The email log is a real log.** It was the last twenty messages kept in a WordPress option, which a shop doing thirty order emails an hour lost by lunchtime. It is a table of its own now: search it by address, subject or the reason something failed; filter by Sent, Server mailer or Failed with a count on each; sort by date or result; page through it at whatever size you like; and see which plugin asked for each message. Keep it for 7, 30 or 90 days or for ever, pruned daily and capped at 20,000 entries so it cannot grow without limit. **Clear the log** empties it behind its own confirmation. The message itself is never stored — only who it went to, its subject, and what became of it. Your existing entries move across on the first page load after updating.
* **The Pop-ups screen matches the rest of the plugin.** Each rule was a grey box with a black tag on it — the one screen that looked like a different product. Every rule is now the same white card used everywhere else, numbered in the order they are matched ("Pop-up 1", "Pop-up 2", because the first one that matches a page is the one that opens), with the Active switch and Remove in the card's header. Eyebrow and Button label share a row with their labels lined up. Adding or removing a pop-up renumbers the rest.
